...
int main()
{
	...
	systemclock_config_HSE(SYS_CLOCK_FREQ_180_MHZ); //we should always do this afetr HAL_Init and before other peripheral usages
	UART2_Init();
...
void systemclock_config_HSE(uint8_t clocl_freq)
{
	...
	osc_init.HSEState = RCC_HSE_BYPASS; //by default it is ON, but anyway, we write it
